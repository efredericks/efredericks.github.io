...
function setup() {
    ...
    main_gfx = createGraphics(width, height);
    background(0);

    for (let _ = 0; _ < 10; _++) {
        let sq = true;
        if (random() > 0.8) sq = false;

        let col = color(random(vaporwave));
        let w = width * 0.75;
        let gfx1 = createGraphics(w * 2, w * 2);
        // gfx1.background(color(220,220,220,80))
        gfx1.noStroke();
        let nr = random(10, 100)
            ;
        let cw = w / nr;
        let i = 0;
        ...
        for (let r = 0; r < nr; r++) {
            for (let c = 0; c < nr; c++) {
                if (i % 2 == 0) gfx1.fill(col);
                else gfx1.noFill();

                if (sq)
                    gfx1.rect(c * cw, r * cw, cw + 2, cw + 2);
                else
                    gfx1.ellipse(c * cw, r * cw, cw + 2, cw + 2);
        ...
        main_gfx.push();
        main_gfx.translate(
            random(width / 3, width * 0.66),
            random(height * 0.25, height * 0.75)
        );
        main_gfx.rotate(random(TWO_PI));
        main_gfx.image(gfx1, 0, 0);
        main_gfx.pop();
    }

    dither_which = int(random(0, 2));
    dither_fs = main_gfx.createFilterShader(dither_src);
    dither_fs.setUniform("which", dither_which);
    main_gfx.filter(dither_fs);
